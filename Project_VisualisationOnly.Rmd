...
subtitle: "AUSSDA hackathon -- Visualisation Only"
...
```{r setup r, warning=FALSE, message=FALSE, echo=FALSE, include=FALSE}
...
```{r Data cleaning 1, warning=FALSE, echo=FALSE}
...
```{r Data cleaning 2, warning=FALSE, echo=FALSE}
...
```{r Raw data visulization, warning=FALSE, fig.width=12, fig.height=10, fig.align='center', echo=FALSE}
data_vi <- data_clean |>
...
model <- glm(far_right_dummy ~ vaccine_status + polintr + female + eduyrs, family = binomial(link = "logit"), data = data_model, weights = round(data_model$pspwght))
...
```{r Simulation visualization 1, warning = FALSE, echo=FALSE}
# UI definition
ui <- dashboardPage(
  dark = TRUE,
